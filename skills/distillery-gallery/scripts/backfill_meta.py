#!/usr/bin/env python3
"""Backfill distillery-meta blocks into existing reports that lack them.

Usage: python3 backfill_meta.py --dir DIR [--dry-run]

Scans DIR for *distillery*.html files without a <script id="distillery-meta">
...
def extract_title(html: str) -> str:
    ...
    if m:
        raw = m.group(1).strip()
        # Remove " — distillery" suffix if present
...
def parse_gen_date(filename: str) -> str:
    """Parse YYYY-MM-DD from filename like 2026-03-06-210126-distillery_*.html"""
...
def backfill_file(path: pathlib.Path, dry_run: bool) -> bool:
    """Add distillery-meta block to a report file. Returns True if modified."""
    ...
    safe_json = json.dumps(meta_obj, ensure_ascii=False).replace('</', '<\\/')
    meta_block = f'{META_SCRIPT_START}{safe_json}</script>'
...
def main():
    ...
    report_files = sorted(scan_dir.glob("*distillery*.html"))
    modified = 0
    skipped = 0
