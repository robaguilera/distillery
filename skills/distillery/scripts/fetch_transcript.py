...
def _fetch_html_metadata(video_id):
    try:
        ...
        if m_pub:
            # Convert YYYY-MM-DD → YYYYMMDD for the shared formatter
            parts = m_pub.group(1)[:10].split("-")
            published = _format_published("".join(parts))

        views = ""
        m_views = re.search(r'"viewCount"\s*:\s*"([0-9]+)"', html)
        if m_views:
            views = _format_views(int(m_views.group(1)))

        duration = ""
        m_dur = re.search(r'"lengthSeconds"\s*:\s*"([0-9]+)"', html)
        if m_dur:
            duration = _format_duration(int(m_dur.group(1)))
...
def main():
    ...
    if lang_pref:
        ...
        for t in tlist:
            if t.language_code == "en" and not getattr(t, "is_translation", False):
                transcript_obj = t
                break
        if transcript_obj is None:
            for t in tlist:
                if t.language_code == "en":
                    transcript_obj = t
                    break
        # Fall back to first non-translated track, then any
        if transcript_obj is None:
            for t in tlist:
                if not getattr(t, "is_translation", False):
                    transcript_obj = t
